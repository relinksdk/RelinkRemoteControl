...
import com.relinkdevice.remotecontrol.util.Constants;
...
public class RemoteIntent extends Intent {
    ...
    public void sendRemote() {
        ...
        if(getPackage() != null && "com.android.bluetooth".equals(getPackage())) {
            Log.i(Constants.TAG, "Sending via bluetooth");
        } else {
            Log.i(Constants.TAG, "Sending via bluetooth");
            Bundle data = new Bundle();
            data.putString(KEY_INTENT, mJSON);
            send(data);
        }
    }
    ...
    private JSONObject saveToJSON() throws JSONException {
        ...
        if (getPackage() != null) {
            obj.put(KEY_PACKAGE, getPackage());
        }
    ...
    public static Intent getFromJSON(String data) {
        ...
        try {
            ...
            if (obj.has(KEY_PACKAGE)) {
                intent.setPackage(obj.getString(KEY_PACKAGE));
            }
